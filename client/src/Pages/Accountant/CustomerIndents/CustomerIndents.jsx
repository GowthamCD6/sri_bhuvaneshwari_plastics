...
import { purchaseIndentService, customerOrderService } from '../../../services/apiService';
...
const filteredIndents = useMemo(() => {
  ...
  if (activeTab === 'opened') {
    ...
    filtered = filtered.filter(indent =>
      indent.status.toLowerCase().includes('delivered') ||
      indent.statusClass === 'badge-delivery-delivered' // Adjust this class if you use a different one
    );
  }
...
const handleViewIndent = (indent) => {
  ...
  navigate('/accountant-purchase-indents', {
    state: {
      ...
      fromAccountantIndents: true,
      readOnly: false
    }
  });
};

const handleOpenDetails = async (indent) => {
  try {
    const orderId = indent.customerOrderId;
    if (!orderId) {
      alert('No associated customer order found for this indent.');
      return;
    }
    ...
    <div className="vsi-main-panel">
      ...
      <div className="vsi-toolbar">
        <div className="vsi-tabs">
          <button
            className={`vsi-tab ${activeTab === 'opened' ? 'active' : ''}`}
            onClick={() => handleTabChange('opened')}
          >
            Opened
          </button>
          <button
            className={`vsi-tab ${activeTab === 'closed' ? 'active' : ''}`}
            onClick={() => handleTabChange('closed')}
          >
            Closed
          </button>
          <button
            className={`vsi-tab ${activeTab === 'delivered' ? 'active' : ''}`}
                ...
                currentIndents.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div className="vsi-actions-cell" style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                        <button
                        ...
                        <button
                          className="vsi-btn-verify"
                          style={{ background: '#f8fafc', color: '#3b82f6', border: '1px solid #bfdbfe' }}
